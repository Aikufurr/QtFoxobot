...
#include "commands/cmd_tictactoe.h"
...
foxobot::foxobot() {
    ...
    //    dev_guild_id = "742736992605044777";
    ...
    // Handles resetting the rank
    RankReset *rankReset = new RankReset(dbManager);
    rankReset->start();
    ...
    connect(client, SIGNAL(message(Client::message_t *)), this, SLOT(message(Client::message_t *)));
...
void foxobot::message(Client::message_t *msg) {
    qDebug() << "Received message from" << msg->author.username << ":" << msg->content;
    if (msg->author.bot) {
        return;
    }
    dbManager->rank_increment(msg->author.id, msg->guild_id);
}
...
void foxobot::interaction_create(Client::interaction_t *interaction) {
    ...
    if (interaction->command == "about") {
        ...
        QObject::connect(cmd, &cmd_about::quit, this, [=](){
            cmd->deleteLater();
            thread->quit();
        });
        ...
        cmd->moveToThread(thread);
        thread->start(QThread::Priority::LowPriority);
        QObject::connect(cmd, &cmd_aesthetics::quit, [=](){
            cmd->deleteLater();
            thread->quit();
        });
        ...
        cmd->moveToThread(thread);
        thread->start(QThread::Priority::LowPriority);
        QObject::connect(cmd, &cmd_avatar::quit, [=](){
            cmd->deleteLater();
            thread->quit();
        });
        ...
        cmd->moveToThread(thread);
        thread->start(QThread::Priority::LowPriority);
        QObject::connect(cmd, &cmd_coinflip::quit, [=](){
            cmd->deleteLater();
            thread->quit();
        });
        ...
        cmd->moveToThread(thread);
        thread->start(QThread::Priority::LowPriority);
        QObject::connect(cmd, &cmd_eight_ball::quit, [=](){
            cmd->deleteLater();
            thread->quit();
        });
        ...
        QObject::connect(cmd, &cmd_hello::quit, [=](){
            cmd->deleteLater();
            thread->quit();
        });
        ...
        cmd->moveToThread(thread);
        thread->start();
        QObject::connect(cmd, &cmd_info::quit, [=](){
            cmd->deleteLater();
            thread->quit();
        });
        ...
        cmd->moveToThread(thread);
        thread->start();
        QObject::connect(cmd, &cmd_leaderboard::quit, [=](){
            cmd->deleteLater();
            thread->quit();
        });
        ...
        cmd->moveToThread(thread);
        thread->start();
        QObject::connect(cmd, &cmd_rank::quit, [=](){
            cmd->deleteLater();
            thread->quit();
        });
        ...
        cmd->moveToThread(thread);
        thread->start();
        QObject::connect(cmd, &cmd_settings::quit, [=](){
            cmd->deleteLater();
            thread->quit();
        });
        ...
        cmd->moveToThread(thread);
        thread->start();
        QObject::connect(cmd, &cmd_mod::quit, [=](){
            cmd->deleteLater();
            thread->quit();
        });
    } else if (interaction->command == "tictactoe") {
        cmd_tictactoe *cmd = new cmd_tictactoe(client, interaction);
        QThread *thread = new QThread;
        cmd->moveToThread(thread);
        thread->start();
        QObject::connect(cmd, &cmd_tictactoe::quit, [=](){
            cmd->deleteLater();
            thread->quit();
        });
...
void foxobot::create_slash_commands() {
...

    {
        ...
        client->me_slash_command_create(command, dev_guild_id);
    }

    {
        QJsonObject command;
        command.insert("name", "about");
        command.insert("description", QString("about %1").arg(client->me_get().username));
        client->me_slash_command_create(command, application_id == dev_application_id ? dev_guild_id : "");
    }
    {
        QJsonObject command;
    ...
    }
    {
        ...
        command.insert("description", QString("Say hello to %1").arg(client->me_get().username));
        client->me_slash_command_create(command, application_id == dev_application_id ? dev_guild_id : "");
    }
    {
        QJsonObject command;
    ...
    }
    {
        ...
        QJsonArray options;
        {
            ...
            }
            {
                ...
                sub_cmd_options.insert("description", QString("If a member activated one of %1'%2 commands").arg(client->me_get().username, client->me_get().username.endsWith("s") ? "" : "s"));
                sub_cmd_options.insert("type", applicationCommandOptionType::SUB_COMMAND);
                QJsonArray sub_cmd_options_options;
                {
                    QJsonObject sub_cmd_option;
    ...
    {
        QJsonObject command;
        command.insert("name", "tictactoe");
        command.insert("description", "Tic Tac Toe!");
        QJsonArray options;
        {
            QJsonObject option;
            option.insert("name", "easy");
            option.insert("description", "Play against an easy AI");
            option.insert("type", applicationCommandOptionType::SUB_COMMAND);
            options.push_back(option);
        }
        {
            QJsonObject option;
            option.insert("name", "medium");
            option.insert("description", "Play against a medium AI");
            option.insert("type", applicationCommandOptionType::SUB_COMMAND);
            options.push_back(option);
        }
        {
            QJsonObject option;
            option.insert("name", "hard");
            option.insert("description", "Play against a hard AI");
            option.insert("type", applicationCommandOptionType::SUB_COMMAND);
            options.push_back(option);
        }
        {
            QJsonObject option;
            option.insert("name", "expert");
            option.insert("description", "Play against an expert AI");
            option.insert("type", applicationCommandOptionType::SUB_COMMAND);
            options.push_back(option);
        }
        command.insert("options", options);
